 ...
 * oldest = #1, newest = #N (based on created_at fallback to meeting_id timestamp)
 */
...
export default function TasksTab({ selectedClientId }) {
  ...
  const canGenerate = useMemo(() => {
    return !!selectedClientId && !!selectedMeetingId && transcriptStatus === "READY" && !hasServerOutputs;
  ...
  async function refreshMeetings({ preserveSelection = true } = {}) {
    ...
    try {
      ...
      if (!preserveSelection) {
        // choose the newest meeting (list is returned in your backend as ScanIndexForward=False,
        // but we won't assume; we pick first if present).
        const nextId = list.length > 0 ? list[0].meeting_id : "";
  ...
  async function onChangeMeeting(nextMeetingId) {
    if (dirty) {
      const ok = window.confirm("You have unsaved edits. Switching meetings will discard them. Continue?");
      if (!ok) return;
    }
    ...
    // Reset transcript panel when switching meetings
    setShowTranscript(false);
    setTranscriptText("");
  }
  ...
  async function onClearTasks() {
    ...
    if (dirty) {
      const ok = window.confirm("You have unsaved edits. Clearing tasks will discard them. Continue?");
      if (!ok) return;
    }

    const ok = window.confirm("Clear tasks for this meeting? This removes the stored tasks and resets task status.");
    if (!ok) return;
              ...
              >
                <div style={{ display: "grid", gap: 10 }}>
                  ...
                  <div style={{ display: "flex", gap: 10, marginTop: 6, flexWrap: "wrap" }}>
                    <button type="button" onClick={onGenerate} disabled={!canGenerate || generating || clearing}>
                    ...
                    <button type="button" onClick={onApprove} disabled={!canApprove || approving || clearing}>
                  ...
                  {!hasServerOutputs ? (
                    ...
                    <>
                      ...
                      <div style={{ marginTop: 16 }}>
                        ...
                        {draftTasks.length === 0 ? (
                          <div style={{ marginTop: 8, opacity: 0.85 }}>No tasks in draft. Add one, or revise with AI.</div>
                        ) : (
                  ...
                  {transcriptStatus !== "READY" ? (
                    <div style={{ marginTop: 14, fontSize: 13, opacity: 0.85 }}>
                      <strong>Note:</strong> You can only generate tasks when <code>transcript_status</code> is{" "}
                      <code>READY</code>. Current: <code>{transcriptStatus || "UNKNOWN"}</code>
                    </div>
                  ) : null}
                </div>
              </div>
